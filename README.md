...
**Manual Alpaca trades won't appear automatically.** To import them:

```bash
# Run from inside the trading-bot container
docker exec -w /app trading-bot python -c "
import sqlite3
...
broker = AlpacaBroker(config, RiskManager(config))
...
for order in orders:
    if str(order.status.value) == 'filled':
        cursor.execute('INSERT OR IGNORE INTO trades (timestamp, symbol, side, quantity, price, total_value, strategy, order_id, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)',
            (datetime.utcnow().isoformat(), order.symbol, str(order.side.value).lower(), float(order.filled_qty), 
             float(order.filled_avg_price or 0), float(order.filled_qty) * float(order.filled_avg_price or 0), 
             'manual', str(order.id), 'Imported from Alpaca'))

conn.commit()
print(f'Imported {len(orders)} trades')
conn.close()
"
```
